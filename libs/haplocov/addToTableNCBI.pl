...
sub diff_d
{
	...
	if ($date eq "NA")
	{
		...
		$diff=0;
		my $diffY=($vl[0]-2019)*365;      # Year, default 2019. Modify this line if need to change
		my $diffM=(int($vl[1]-12)*30.42); # Months, default 12. Modify this line if need to change
		my $diffD=$vl[2]-30;		  # Day, default to 30. Modify this line if need to change	
	...
	for (my $i=0;$i<$numP;$i++)
		...
		push(@outfiles,"$file\_variants");
                unless (defined($pid))
                {
                        ...
                        exec("perl align.pl --infile $file --ref $ref --outfile $file\_variants") || die "can't exec $!";
                        exit(0);
                }else {
                        push(@childs,$pid);
                }
