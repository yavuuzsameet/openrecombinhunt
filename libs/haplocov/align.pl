...
sub align
{
	...
	$seq=fix_seq($seq);
        ...
        print OUTFA ">$strain\n$seq\n";
	#print "$strain\n";
	system("nucmer -l 9 --maxmatch -c 50 --prefix=$randTempName $ref $genomesF/$strain.fasta 2> /dev/null")==0||die("$strain: alignment failed\n");
	#print "alignDone: nucmer -l 9 --maxmatch -c 50 --prefix=$randTempName $ref $genomesF/$strain.fasta\n";
	...
	if ($arguments{"--ref"} eq "na" ||  (! -e $arguments{"--ref"}))
        {
                print_help();
                my $f=$arguments{"--ref"};
                die("No valid reference file provided. $f does not exist!");
        }
...
sub print_help
{
	...
	print "--ref <<filename>>\t provides a fasta file with the reference genome\n";
	...
	print "--outfile <<filename>>\t name of the output file. Defaults to varcall.list\n";
        ...
        print "\n##EXAMPLE:\n\n";
        print "1# input is multi-fasta (apollo.fa), reference is a fata file (myref.fa):\nperl align.pl --infile apollo.fa --ref myref.fa\n\n";
}
